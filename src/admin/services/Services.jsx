...
const  Services = ()=> {
  ...
  return (
    <section id="services-management" className="page-section min-h-screen bg-gray-50 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2 sora-family">Services Management</h1>
        <p className="text-gray-600 inter-family">Manage your portfolio services and offerings</p>
      </div>
      ...
      <div className="mb-6">
        <button
          ...
          transition-colors flex items-center inter-family"
        >
        ...
        {servicesData.map(service => (
          <div key={service.id} className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <div className={`w-12 h-12 ${service.iconColor} rounded-lg flex 
              items-center justify-center`}>
                <svg className={`w-6 h-6 ${service.iconStroke}`} fill="none" stroke="currentColor" 
                viewBox="0 0 24 24">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2 sora-family">{service.title}</h3>
            <p className="text-gray-600 text-sm mb-4 inter-family">{service.description}</p>
            <div className="flex items-center justify-between">
              <span className="text-sm text-green-600 bg-green-100 px-2 py-1 rounded-full inter-family">{service.status}</span>
              <span className="text-sm text-gray-500 inter-family">{service.price}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {showModal && (
        ...
        justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 transform transition-all">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900 sora-family">Add New Service</h3>
              <button onClick={closeServiceModal} className="text-gray-400 
            ...
            <form className="space-y-4 inter-family">
              <div>
